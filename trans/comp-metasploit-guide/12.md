...
| `Msf::Auxiliary::WmapScanServer` | `/lib/msf/core/auxiliary/wmapmodule.rb` | 你可能会想，什么是 WMAP？WMAP 是 Metasploit 框架的一个基于 web 应用的漏洞扫描插件，它有助于使用 Metasploit 进行 web 测试。 |
...
让我们首先了解 MeterMeter 脚本的基础知识。使用 MeterMeter 编码的基础是**应用编程接口**（**API**调用和混合。需要使用特定的基于 Windows 的**动态链接库**（**DLL】**执行特定任务，并使用各种内置的基于 Ruby 的模块执行一些常见任务。
