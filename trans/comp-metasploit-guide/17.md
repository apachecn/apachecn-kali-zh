...
![](assets/af0e3d45-40f4-46b8-8943-71d8f3a0fe42.png)
...
![](assets/1898ad2e-6e22-423e-bf5e-92f8a0aa073b.png)
...
![](assets/d47c58f9-d31a-467b-b2e0-d36c724afc31.png)

前面的图表把事情讲得很清楚。现在让我们来看看如何做。但请记住，此攻击最重要的要求是访问具有适当权限的易受攻击服务器。让我们通过以下屏幕截图了解有关注入恶意脚本的更多信息：

![](assets/69db68ca-2c5b-42cb-bf8a-a419149329af.png)
...
![](assets/d0a36848-5ab7-4e7c-a6a8-1685ca963f2d.png)
...
![](assets/871b6a25-7d7c-4dcf-accb-858c3528ace9.png)

我们可以看到对 IP`192.168.10.107`进行了调用，这是我们的浏览器 autopwn 服务器。让我们从攻击者的角度看一下视图，如下所示：

![](assets/470284b3-7f5d-4d16-9853-8ab12baa9562.png)
...
![](assets/ac92e6ad-e001-436e-94f3-5cddee1b7d79.png)
...
![](assets/8934ff24-fe87-4ac6-a09a-502c28151de8.png)

让我们在端口`80`上启动我们的恶意`browser_autopwn`服务：

![](assets/1d65b390-e000-4596-b44d-ce92de05b118.png)

现在，让我们看看当受害者试图打开[时会发生什么 http://google.com/](http://google.com/) ：

![](assets/923abff6-f76b-4fe2-b0e2-5a78860e8ac8.png)

让我们看看攻击者方面是否有什么有趣的东西：

![](assets/52c01a0b-36e2-42a0-9603-4b6ebc8f9648.png)
...
![](assets/95c5e863-7d54-4cc0-bd80-45bd7041b3c3.png)
...
![](assets/a219f45f-6784-487c-82e1-7eb195fb3e0b.png)

我们假设我们的目标是`DESKTOP-PESQ21S`点击它将打开一个子菜单，其中包含列出的所有选项：

![](assets/28e22458-77d1-44d4-b0d3-a592c98d11fc.png)

让我们选择 MITM，然后选择脚本注入和自定义代码，这将导致以下屏幕：

![](assets/b91ce379-7a05-41a9-9f34-965a27679df8.png)

我们将使用自定义脚本攻击和默认脚本开始。现在，它将把这个脚本注入到目标浏览的所有网页中。让我们按 OK 启动攻击。一旦目标打开新网站，受害者将收到以下信息：

![](assets/71587fdc-23ca-4556-92d0-1e7e77b4d3cc.png)
...
![](assets/93235454-fb60-409a-bedd-96f0d8188d91.png)

太棒了，对吧？NetHunter 和 cSploit 是游戏规则的改变者。但是，如果您无法创建 JavaScript，可以使用重定向选项重定向目标，如下所示：

![](assets/1a27093b-9793-4e0a-a676-661c7cdc5b15.png)
...
![](assets/805955d7-654d-41b1-a4c5-153611bc715b.jpg)
...
![](assets/60db5ac4-e456-44a9-be61-089f6f46890f.png)

我们可以看到，我们必须通过浏览工具菜单来选择电路板类型，如前面的屏幕截图所示。接下来，我们需要为电路板选择通信端口：

![](assets/48473637-2cad-4666-a1bd-e0317a13d2e6.png)

接下来，我们需要通过按->图标将程序写入电路板：

![](assets/0b48c16d-a8e2-4b7a-991c-88cebff09851.png)

我们的 Arduino 现在已经准备好接入受害者的系统。好消息是它模拟了键盘。因此，您不必担心检测问题；然而，有效载荷需要足够好的模糊处理，以避免 AV 检测。

按如下方式插入设备：

![](assets/5e5af455-79e3-4533-af20-750fcbbf1590.jpg)

一旦我们插入设备，在几毫秒内，我们的有效负载就会被下载，在目标系统上执行，并向我们提供以下信息：

![](assets/3a9f2dae-4f3e-49d8-8cf4-57e79493cbfb.png)

让我们看看我们是如何生成有效负载的：

![](assets/e3acb414-54c6-428e-84ee-ecc40da7ce96.png)
...
![](assets/9047da23-c2d0-4452-ac55-63c27bf4cfa2.png)
...
![](assets/1deb1462-865f-4d77-98af-ae64aa9b6c63.png)

在几分之一秒内，重叠窗口将消失，并将产生一个成功的流量计外壳，如以下屏幕截图所示：

![](assets/5ea1162a-a9e8-4015-a241-b6386f513749.png)
...
![](assets/4c8845b2-71db-4f51-9e19-10f9d2e3e99d.png)

我们将`FILENAME`和`SRVHOST`参数分别设置为`Report.doc`和我们的 IP 地址，如下图所示：

![](assets/0698f567-a9c4-454b-a423-7f54b66e54ac.png)

生成的文件存储在`/root/.msf4/local/Report.doc`路径中。让我们将此文件移动到 Apache`htdocs`目录：

![](assets/ce9289e2-89a4-4269-aa05-c9628e20aa43.png)
...
![](assets/62a25f69-a9c3-4d83-872a-7d6967238dce.png)

让我们打开此文件并检查是否发生了某些事情：

![](assets/cfa24d6a-0d90-4b1f-b4b4-21b4cc699ab0.png)

我们看不出这里发生了什么事。让我们回到我们的 Metasploit 控制台，看看我们是否得到了一些东西：

![](assets/3a219fea-6305-4fa8-9b08-92bd2fd18f04.png)
...
![](assets/a6c135fd-7c18-4d05-aede-dcaec72610bb.png)

在生成 APK 文件时，我们需要做的就是说服受害者（执行社会工程）安装 APK，或者实际访问手机。让我们看看受害者下载恶意 APK 后手机上会发生什么：

![](assets/7eaa041d-4e0e-4115-9ccb-c9f6a7910c4e.png)

下载完成后，用户将按如下方式安装文件：

![](assets/d9ebb820-b4b5-4318-b360-500c999f7004.png)

大多数人在智能手机上安装新应用程序时，从未注意到应用程序要求什么权限。因此，攻击者可以完全访问手机并窃取个人数据。前面的屏幕截图列出了应用程序正确运行所需的权限。一旦安装成功，攻击者就可以完全访问目标手机：

![](assets/46575e06-9f78-4eac-a0e0-bd57c55b21e2.png)

哇！我们可以很容易地进入计量器。下一章将广泛讨论后剥削问题；但是，让我们看看一些基本功能：

![](assets/39e3fc1b-9ac2-41db-936b-b780e749d90f.png)

我们可以看到，运行`check_root`命令表示设备已根化。让我们看看其他一些函数：

![](assets/16336ba2-8434-4dcb-9f86-0743f0c1c8d1.png)

我们可以使用`send_sms`命令从被攻击的手机向任何号码发送短信。让我们看看消息是否已传递：

![](assets/d515fdf6-33e4-4bac-9c00-0d68772e17e1.png)

答对 了消息已成功传递。同时，让我们看看我们使用`sysinfo`命令闯入了什么系统：

![](assets/a68d758a-139e-4f60-871b-4ba04ae54042.png)

让我们定位移动电话：

![](assets/57c4bd84-9e55-4cd6-8957-60ff7555654a.png)

浏览谷歌地图链接，我们可以获得手机的确切位置：

![](assets/0879a63b-aed1-4a9b-aa99-f640e3435960.png)

让我们用手机的摄像头拍摄一些照片：

![](assets/64f00608-de17-4222-8901-45ca7e320564.png)

我们可以看到我们从照相机里得到的照片。让我们来看看图片：

![](assets/51d63ae1-66b8-43d9-b720-714248135412.png)
