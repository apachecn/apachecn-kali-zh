...
![](assets/3c784529-c763-45f4-b9f6-986da6b7dfaa.png)
...
![](assets/d19df6ba-558b-4815-b73a-7fcfbbadf834.png)

由于`db_nmap`扫描填充了大量结果，让我们看看启用了哪些`minion`选项：

![](assets/4a6bf20d-da16-4751-af70-87f82c4ae853.png)

大量我们可以看到目标主机上有 MySQL 服务。下面我们使用`mysql_enum`命令：

![](assets/a529f592-5dca-45f5-8e18-2c2148a07fc8.png)

哇！我们从不需要加载模块、填写任何选项或启动模块，因为`minion`插件为我们实现了自动化。我们可以看到目标主机的 MySQL 版本。下面我们使用`minion`中的 MySQL 攻击命令：

![](assets/eb7c6aac-5de9-4ae6-9ec8-151805809f36.png)
...
![](assets/93733b15-1002-4f0e-bc68-c394777821dd.png)
...
![](assets/209a37d8-732e-400b-a0a4-89fb83a2c038.png)

正如我们所看到的，我们打开了一个命令 shell，它总是希望有更好的控制访问，如 MeterMeter 提供的访问。在这种情况下，我们可以使用`-u`开关升级会话，如下图所示：

![](assets/7e06feeb-56fe-4d9e-9847-d6816e729dcc.png)
...
![](assets/ca2a63aa-c451-4a25-9138-ffbfbf4735b0.png)

我们可以使用`-n`开关命名一个 shell，如下面的屏幕截图所示：

![](assets/2c8f481a-638d-477d-a0dd-4f8f1655c942.png)
...
![](assets/a94735f9-ea54-4c48-ab0e-6d4e6957a9e7.png)

此外，您始终可以执行以下屏幕截图所示的操作：

![](assets/54b029de-89e7-453e-9e83-f7bd9d5f9d2a.png)
...
![](assets/01c5eab5-a9c9-4585-a0eb-c3af2f2d2af5.png)

让我们启动 Metasploit，看看是否成功保存了上一次会话中的所有内容：

![](assets/eb0437b6-ef22-415e-b097-76bbdccd2040.png)
...
![](assets/a409d10f-689f-4dfe-96fa-6555781b08c7.png)

在前面的屏幕截图中，我们将`se-script`输入`seautomate`工具，从而生成有效负载并自动设置漏洞处理程序。让我们更详细地分析一下`se-script`：

![](assets/d0b90fcf-42d3-47c7-a485-8628e2007786.png)
...
![](assets/3141c845-a8b1-4e17-ad39-3c93b99160b9.png)

脚本中的下一个数字是`4`。因此选择了`Create a Payload and Listener`选项，如下图所示：

![](assets/33953706-5a0a-4042-8a3c-dfa166aaebca.png)

下一个数字是`2`，表示有效负载类型为`Windows Reverse_TCP Meterpreter`，如下图所示：

![](assets/8e158ee8-8be1-46f6-971a-8f61dc9282d1.png)

接下来，我们需要指定侦听器的 IP 地址，即脚本中的`192.168.10.103`。这可以手动可视化：

![](assets/14c1cec9-5e37-4941-8606-b4d144aa3c8b.png)

在下一个命令中，我们有`4444`，它是侦听器的端口号：

![](assets/1a955100-20c3-4f31-a874-4d6da9c61a82.png)

我们有`yes`作为脚本中的下一个命令。脚本中的`yes`表示侦听器的初始化：

![](assets/fcde75ae-a39a-4b71-99d9-3b2d664e6d2d.png)

一旦我们提供`yes`，控制权就转移到 Metasploit，漏洞反向处理程序将自动设置，如下图所示：

![](assets/b2a54ba0-9515-4e0a-a885-2a5893ac52c3.png)
