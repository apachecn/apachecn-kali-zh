...
在本章中，我们将介绍Armitage及其特点。我们还将研究如何使用这个支持 GUI 的 Metasploit 工具进行渗透测试。在本章的后半部分，我们将研究 Armitage 的 Cortana 脚本。
...
通过单击相应的按钮，我们可以轻松地在目标系统上上载、下载和查看所需的任何文件。这就是Armitage的美；它远离命令，并以图形格式显示所有内容。
...
我们可以看到，我们已成功地从本地Armitage实例连接到团队服务器。此外，所有连接的用户都可以通过事件日志窗口相互聊天。考虑到我们有另一个用户加入了团队服务器：
