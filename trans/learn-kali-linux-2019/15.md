...
![](assets/f1717a7a-4a87-4e02-be65-02f316d8184e.png)

3.  导航到以下页面：OWASP 2013 | A2-中断的身份验证和会话管理|身份验证绕过|通过 SQL 注入|登录：

![](assets/b115b27e-7329-4a6d-815a-f394dfec0cfd.png)
...
![](assets/a2d052df-0d22-4666-b476-ca23429aed7c.png)
...
![](assets/8dcdeaa1-17d5-45c9-b3d3-d58f1ebfd0c3.png)

语句选择表中的第一条记录并返回它。在检查登录状态后，我们可以看到我们现在以`admin`的身份登录。这意味着第一条记录是`admin`：

![](assets/0aeb18ef-d3fe-4a0f-bc9e-76d3073cd1b4.png)
...
![](assets/50e354e6-c4cd-4212-9c8a-0ac4cb645a50.png)
...
![](assets/4dc69b92-0578-4865-a8c4-ed1fe4953881.png)

4.  使用`bee`作为用户名，使用`bug`作为密码登录应用程序。然后单击登录：

![](assets/25357337-5c86-4788-9715-1c418deaeba8.png)

5.  如图所示选择**SQL 注入（搜索/获取）**选项，点击**Hack**继续：

![](assets/367d8a9e-363e-4285-b6a8-026ae4861c92.png)

6.  将出现一个搜索框和表。在搜索字段中输入数据时，GET 请求用于从 SQL 数据库检索信息并将其显示在网页上。现在，让我们搜索包含字符串`war`的所有电影：

![](assets/82a4d2aa-7ecd-40aa-85b3-f53c20dc10d6.png)
...
![](assets/98184d34-39c8-4867-b10e-e14983e270a6.png)
...
![](assets/e775c7de-ddd0-4fee-9860-7bcdeea5202e.png)
...
![](assets/323bc279-8d5e-47ba-bb84-da0593b1a62a.png)
...
![](assets/98ad17d6-f08d-4d9e-84de-871bb24941c2.png)
...
![](assets/dd33688b-c824-4c75-ae8a-c456bba50eac.png)
...
![](assets/d7ed96cb-695a-4fa9-9cc5-ae2cad9596fc.png)

现在，我们有了数据库中的所有表。以下表格由开发人员创建：

![](assets/d83a745e-bd75-4e27-8ff2-f9e026eec4e7.png)
...
![](assets/ab801668-4069-4176-ad37-e2c2795ca39d.png)
...
![](assets/532081a3-3b69-4443-83e4-11720942af8a.png)
...
![](assets/74cefbdc-7413-40e9-aab8-039535ef7067.png)
...
![](assets/252ee273-f27c-4cc8-9214-36fe87ab8050.png)

5.  在右上角，使用下拉菜单选择**SQL 注入（搜索/发布）**，然后点击**黑客**加载漏洞：

![](assets/27bd4782-4a1e-467c-b04f-ae15ab3b14c2.png)

6.  在搜索字段中输入一个单词，点击**搜索**提交（发布）数据：

![](assets/341bfc3c-0011-4442-b037-690bc2042362.png)
...
![](assets/6be97d31-cd41-499b-84cd-fcbea98e164b.png)

以下为本**帖子**的内容：

![](assets/6411e822-e04b-430d-b2b2-939a1e3db431.png)
...
![](assets/0cff3574-e7c6-4bbf-ae1e-385b1cb77036.png)

在前面的屏幕截图中，SQLmap 能够注意到`'title'`参数可能有漏洞，数据库也可能是 MySQL 平台。此外，以下是已找到的可注入参数的示例：

![](assets/0a9562f0-5ef4-4915-9f2c-03f451eaef02.png)

前面的屏幕截图显示，SQLmap 已确定`'title'`参数也容易受到 SQL 注入攻击。最后，以下是 SQLmap 有效负载：

![](assets/6922c792-ea44-4015-a8cd-20dd2b02106e.png)
...
![](assets/74a30c5d-1318-4654-ab44-db4f38e2d253.png)
...
![](assets/4a7b6e9b-a189-4c8d-bdaa-2e4c2bcf6251.png)

3.  不在表单中输入任何详细信息，点击**进入**。查看 web 浏览器地址栏中的 URL，您可以看到 URL 可以编辑：

![](assets/84b995db-5825-4449-b660-8bab450a5653.png)
...
![](assets/02349b59-a85d-47ab-93fe-ad984c56906b.png)

5.  点击**Go**在服务器上执行脚本。将出现以下弹出窗口：

![](assets/de4069fa-a416-4839-be7b-fb294cf4a768.png)
...
![](assets/c8c0b4d1-83fd-4c93-b35d-b6686329f9c2.png)

3.  您可以在文本字段中输入任何消息，然后单击提交。输入的文本现在将存储在数据库中，如在线留言板、论坛或带有评论部分的网站：

![](assets/2d5bd725-8ad3-46f5-9eab-2b9409a0ac4f.png)
...
![](assets/b8b74ea1-6c52-4702-9d03-113931a1d321.png)

查看表格，第二行没有任何实际条目：

![](assets/9b131d08-9ccf-4012-bf1a-6a1c1239211b.png)
...
![](assets/6189fb95-6088-4d4d-9b43-31783dfee3a4.png)

让我们开始使用 BeEF 来利用 XSS 漏洞：

1.  要打开 BeEF，请转至**应用程序****08–开发工具****BeEF xss 框架**。BeEF 服务将启动并显示以下详细信息以访问 BeEF 界面：

![](assets/18de4963-6b2f-4483-95fe-b19301d2ca72.png)
...
![](assets/ee2553fa-1934-4107-80cf-cae16a0952fa.png)
...
![](assets/dd2248ee-734a-45ce-9063-29fe5a322ba2.png)

3.  打开打嗝套件并确保**截取**已打开。
4.  在 DVWA 网页上，点击**提交**按钮向服务器发送 HTTP 请求：

![](assets/0a900c70-50af-4042-95aa-cf9952f9ec1d.png)

5.  在 Burp 套件中，您应该能够看到 HTTP 请求。右键单击上下文窗口中的任意位置，选择**进行活动扫描**：

![](assets/8370b5cf-c944-47c5-8467-f236801b1ab5.png)

这将允许 Burp Suite 在目标网页上执行自动扫描，以发现任何 web 漏洞。

以下是使用 Burp Suite 完成扫描后的结果示例：

![](assets/edf2e386-d418-4b5c-963c-a6c202b7f049.png)
...
![](assets/999655ad-e166-4d03-85b6-894acf563d4d.png)
...
![](assets/1c66772c-aefe-47bf-a663-6fa7a54bffe2.png)

4.  在命令行界面上，通读并接受**最终用户许可协议**（**EULA**）。
5.  在 Kali Linux 中打开 web 浏览器并输入以下地址`https://kali:13443/`，以访问 Acunetix 用户界面。使用安装过程中创建的用户帐户登录：

![](assets/168e73b6-2d46-47a3-ab0d-be81b69ab400.png)

6.  要开始新的扫描，点击**创建新目标**或**添加目标**，如下所示：

![](assets/11b79283-7d0f-44d3-9ed2-544185c8ae28.png)

7.  将打开**添加目标**弹出窗口，可指定目标：

![](assets/f7240a89-ff37-42d9-8731-c9ca8faed639.png)

8.  添加目标后，将向您提供自定义扫描的选项：

![](assets/f4b4bd21-dab9-4f71-951c-9315ec2ee22a.png)

现在，我们将保留所有选项的默认设置。

9.  指定扫描类型和报告选项：

![](assets/ee4c16f7-db4f-4ead-a9f4-7e8cadf04b1d.png)
...
![](assets/2061bc20-6840-422d-a5ab-51479a9db37a.png)

2.  输入 OWASP BWA 虚拟机的 IP 地址，然后单击“攻击”开始安全扫描：

![](assets/56e0d854-2bfc-443e-ae4c-24fd1c2c81c6.png)

在扫描阶段，OWASP ZAP 将在目标上执行爬网。**爬行**是一种网络安全扫描程序检测隐藏目录并尝试访问它们（爬行）的技术：

![](assets/a836afd6-7964-41d5-8651-916ef2632caf.png)

3.  扫描完成后，单击“警报”选项卡以查看找到的所有基于 web 的漏洞以及每个漏洞在目标上的位置：

![](assets/8ba840a6-1592-4fbc-a5c7-5971d905df1e.png)

选择漏洞后，OWASP 将在从目标服务器返回 HTTP 头和正文时显示它们：

![](assets/9eb1000e-c212-4d9b-9706-8f2a5378778f.png)
...
![](assets/7f01fa60-3283-4f1f-9f7b-ec0d599cd970.png)
