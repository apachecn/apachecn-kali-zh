...
![](assets/edcf143a-1eb8-44ce-b001-e0f59b5f229c.png)
...
![](assets/5f33b307-ee03-4cb4-8ed2-f4d10fc4b5b1.png)
...
![](assets/bc9e7142-0794-46bb-bdf8-322a513388d9.png)
...
![](assets/985cb9a0-5002-4993-8d7c-8a3e7d86131f.png)

6.  我们可以看到程序崩溃了，显示错误`0x41414141`。这意味着我们输入的字符`A`已经覆盖了 EIP。

7.  我们通过键入`i r`进行确认：

![](assets/572fdc5f-2ff5-4bba-b6b4-48eef12325d2.png)
...
![](assets/8a365f60-2e6a-4b5a-bfd1-75914c58fb8b.png)

15.  我们在调用函数的行中添加断点，然后使用`b <linenumber>`调用函数。

16.  现在我们再次运行程序，它将在断点处停止：

![](assets/42088718-8f43-41a1-b036-46c996900a12.png)
...
![](assets/d1300130-681f-494a-8359-b08e0d3730e9.png)
...
![](assets/e448340f-ef70-4c50-8128-34e5fff7bfec.png)

21.  让我们尝试利用溢出打开一个 shell。我们可以在 Google 上找到将为我们执行 shell 的外壳代码：

![](assets/c42bb2b9-3caa-4f65-8a04-43f7eff0c8e1.png)
...
![](assets/2237b2f9-0aa0-4912-80e7-01c65bcc2b91.png)

4.  然后将文件拖放到播放器中，我们将看到它崩溃：

![](assets/24084d91-a809-4a7c-a5e6-bb289d7855aa.png)
...
![](assets/ff14a5e0-2a9f-4c2a-8ac8-4b2cf31e5ee6.png)

9.  现在，我们运行调试器并通过导航到文件| attach:

![](assets/d6ee96ff-ee75-41e1-9695-f6e2d1bb137d.png)

10.  然后，我们从正在运行的程序列表中选择进程名称：

![](assets/37bb1dd1-3b80-4193-8bc9-d4198c05a5e5.png)

11.  连接后，我们将在程序中打开 M3U 文件。我们将在调试器的状态栏中看到一条警告。我们只需按*F9*键或点击顶部菜单栏上的播放按钮，点击继续：

![](assets/2b1bd8d3-d2fb-4640-8104-cbfe0677069d.png)

12.  我们将看到 EIP 被`A`s 覆盖，程序崩溃：

![](assets/57f2063e-3b69-42d4-a4ce-3df272fdadad.png)
...
![](assets/c52b2870-f2a2-42dd-91f8-51a6a93423f0.png)
...
![](assets/9da7ca5f-52ae-4996-abf2-c5e624cfe55c.png)
...
![](assets/a66fd3d6-f421-4b69-8373-154c3d24ad26.png)
...
![](assets/f66e9a99-f581-456c-95d9-a005e49b0e74.png)
...
![](assets/bb49615b-16b0-447a-bbba-38ac7012266e.png)

36.  在结果中，我们看到地址中有序列：`018F1D88`。

37.  现在，我们只需用此代码替换攻击代码中的 EIP 地址，然后运行攻击，我们应该打开一个计算器：

![](assets/b6a07761-4b4d-4742-b69f-aa38900d5c6c.png)
...
![](assets/6f9ef080-d0b9-4e2f-86c9-86aafce20782.png)
...
![](assets/ea4fd8f1-96bc-4169-b405-16cdfaa4739f.png)

4.  让我们通过打开软件并导航到服务器来运行 AntServer |运行服务控制…：

![](assets/110334f7-16d2-4b7d-9747-fe7014451572.png)
...
![](assets/a4e6fef0-c9bd-4a0a-ab80-631458aa0a24.png)
...
![](assets/2a92070e-e7f6-4ebc-8139-733ababfc285.png)

15.  代码应该是这样的：

![](assets/79e71616-6b73-4c64-8b4a-8c51bfda71b0.png)

16.  我们现在运行这个文件，在调试器中，我们将看到访问冲突错误。我们现在来看| SEH 连锁店。
17.  我们将看到 SEH 已被字节覆盖。我们复制`42326742`值，并使用 Kali 中的`pattern_offset`脚本找到其位置：

![](assets/627727a0-1665-449e-b580-bf31f157d06f.png)
...
![](assets/09a3a41d-f031-4c46-b222-106ce855f3e9.png)
...
![](assets/9923fd87-3710-4aa9-ac11-2a4873962b40.png)
...
![](assets/e5fe1f24-019f-47fb-b5ac-841de81bcc5e.png)

23.  这将向我们显示未使用`SAFESEH/ON`编译的所有 DLL 的列表。在日志窗口中，我们将看到函数列表：

![](assets/6732dd8a-5f97-4211-97bb-3ba05ebc9e64.png)
...
![](assets/227092fd-5f40-4a32-b23e-d3b72b31bfb9.png)
...
![](assets/4a72551c-1e19-414f-a9f1-33d95e744367.png)
...
![](assets/7a5ad769-7b63-423a-8e12-8068eeaf83ec.png)

30.  这次让我们在没有调试器的情况下运行它。我们将在 Kali 打开我们的处理程序，我们应该有 MeterMeter 访问权限：

![](assets/44862fbb-f064-4d32-b21d-026f52f9e59d.png)
...
![](assets/3eba1c6c-372e-4d0e-850e-96b87d14b964.png)
...
![](assets/a58a884f-8baa-43f0-b726-980a8d468454.png)
...
![](assets/98bb2d9f-1042-4142-a474-9d170c4f0a7f.png)
...
![](assets/84aa6fc8-e246-4289-ae33-e655f26d184b.png)
