...
在浏览器中执行的客户机上可用的代码需要进行测试，以确定是否存在敏感信息或是否允许用户输入，而无需服务器端验证。了解如何使用Burp执行这些测试。
...
*   **OWASP 断网应用**（**VM**）
*   **OWASP 多联线**
*   **Burp代理社区**或**专业人士**（[https://portswigger.net/burp/](https://portswigger.net/burp/)
...
2.  切换到Burp，然后从顶级菜单中选择Burp单击Burp：
