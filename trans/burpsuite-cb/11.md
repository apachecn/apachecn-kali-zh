...
11.  切换到Burp入侵者|位置选项卡。清除所有建议的有效负载标记并突出显示 IP 地址，单击*添加§*按钮在`target_host`参数的 IP 地址值周围放置有效负载标记：
...
Web 渗透测试人员必须确保处理 AJAX 调用的应用程序（例如 HTML5）没有错误配置的 CORS 头。让我们看看Burp如何帮助我们识别这些错误配置。
...
*   要了解更多关于这个Burp插件的信息，请查看[https://blog.netspi.com/java-deserialization-attacks-burp/](https://blog.netspi.com/java-deserialization-attacks-burp/)